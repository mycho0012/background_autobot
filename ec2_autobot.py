...
def main():
    ...
    # Initialize the bot once to send the start message
    try:
        bot = YingYangTradingBot('KRW-BTC', 'minute30', 300, stop_loss_percentage=5, take_profit_percentage=10)
        start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        bot.send_slack_message(f"YingYang Trading Bot started at {start_time} with 30-minute intervals, 5% stop loss, and 10% take profit")
    except Exception as e:
    ...
    try:
        bot.send_slack_message("YingYang Trading Bot stopped")
    except Exception as e:
